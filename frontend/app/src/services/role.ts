...
export const roleService = {
  ...
  createRole: async (payload: RolePayload): Promise<RoleSummary> => {
    const response = await api.post('/roles', payload, authConfig());
    return unwrapData<RoleSummary>(response);
  },

  updateRole: async (roleId: number | string, payload: Pick<RolePayload, 'description'>): Promise<RoleSummary> => {
